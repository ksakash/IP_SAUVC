...
bool IP = false;
int b1min, b1max, b2min, b2max, b3min, b3max;  // Default Params
...
void callback(IP_SAUVC::ballConfig &config, uint32_t level)
{
  ...
  b2min = config.b2min_param;
  b2max = config.b2max_param;
  b3min = config.b3min_param;
  b3max = config.b3max_param;
